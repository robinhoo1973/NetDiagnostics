...
Item {
    ...
    function diagDesc(id) {
        ...
        var descs = {
            ...
            18: t("VPN status detection: cross-references GeoIP country with actual server reachability", "Statut VPN : recoupement GeoIP et accessibilité réelle", "VPN-Status: Abgleich GeoIP-Land mit Server-Erreichbarkeit", "Статус VPN: сопоставление GeoIP с доступностью серверов", "Stato VPN: incrocio paese GeoIP e raggiungibilità server", "VPN状态: 交叉比对GeoIP国家和服务器可达性", "VPN狀態: 交叉比對GeoIP國家和伺服器可達性", "Estado VPN: cruce de país GeoIP con accesibilidad real", "Status VPN: cruzamento de país GeoIP com acessibilidade real"),
