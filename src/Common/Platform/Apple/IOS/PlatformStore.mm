...
// 5WHY (iOS b21294): a second restore request arriving while one is already
// in flight (auto-probe + a manual tap) was previously answered immediately
// with callback(false,false) — a false "no purchase found".  Queue the new
// callback here and resolve it with the CURRENT restore's result instead.
...
- (void)startRestore {
    ...
    self.hasRestored  = NO;
    // 5WHY (iOS b21294): restoreCompletedTransactions may never call back if
    // there is no network / the App Store is unreachable — neither
    // paymentQueueRestoreCompletedTransactionsFinished nor the ...FailedWithError
    // delegate fires.  Without a watchdog the C++ side's m_purchaseInProgress
    // stays true forever, silently disabling BOTH the Buy and Restore buttons
    // (PremiumStore guards them with `if (m_purchaseInProgress) return;`).
    // Mirror the purchase watchdog: fail the restore after 30s so the UI
    // recovers and the user can retry.
    __weak typeof(self) weakSelf = self;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(30 * NSEC_PER_SEC)),
                   dispatch_get_main_queue(), ^{
        typeof(self) strongSelf = weakSelf;
...
// 5WHY (iOS b21294): auto-probe + manual tap can overlap.  Resolve both the
// active onRestoreDone and any pendingRestoreDone with the same result so a
// queued manual tap never gets a spurious "no purchase found".
...
void platformRestorePurchases(RestoreCallback callback) {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        // PremiumDialog, or a prior manual tap) must NOT abort the new request
        // with a spurious callback(false,false).  Queue it and resolve with
        // the current restore's outcome (see resolveRestoreWithResult).
        if (obs.isRestoring) {
