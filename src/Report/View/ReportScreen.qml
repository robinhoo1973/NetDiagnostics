...
Item {
    ...
    // 5WHY: Share called exportPdf/exportHtml again, generating a second
    // copy of the same report. Now reuses the preview file (already
    // generated by openPreview). Files are deleted when preview is dismissed.
    // User confirmed sharing (premium path). Shares the EXISTING preview file.
    function confirmShare() {
        var fmt = pendingShareFormat
        shareStage = 0
        var path = (fmt === "pdf") ? page.previewPdfPath : page.previewHtmlPath
        if (path) {
            appState.shareExistingReport(path, fmt)
        } else {
