...
DiagnosticResult mysqlDiagnostics(const QString& target) {
    ...
    if (!probe.connected) {
        auto r = result(DiagId::G5Mysql, "Connection failed", DiagStatus::Fail,
                      {}, probe.elapsedMs);
        r.data["host"] = u.host();
        r.data["port"] = port;
    ...
    if (data.size() < 5) {
        auto r = result(DiagId::G5Mysql, "No handshake packet", DiagStatus::Warning,
                      {}, probe.elapsedMs);
        r.data["host"] = u.host();
        r.data["port"] = port;
